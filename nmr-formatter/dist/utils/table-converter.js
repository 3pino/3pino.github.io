...
exports.tableToData = tableToData;
exports.getMaxJValues = getMaxJValues;
...
function dataToTable(data) {
    ...
    logger_1.Logger.debug('Converting NMR data to table:', data);
    ...
    logger_1.Logger.debug('Generated table:', table);
    return table;
}
// Convert table structure back to NMRData
function tableToData(table, metadata = {}) {
    if (!table || table.length < 2) {
        // Return empty NMRData if table is invalid
        const completeMetadata = new Metadata_1.Metadata(metadata.nuclei || "", metadata.solvent || "", metadata.frequency || 0);
        return new NMRData_1.NMRData([], completeMetadata);
    }
    logger_1.Logger.debug('Converting table to NMR data:', table);
    ...
    logger_1.Logger.debug('Column indices - shift:', shiftIndex, 'mult:', multIndex, 'integration:', integrationIndex, 'jValues:', jValueIndices);
    // Process each data row
    dataRows.forEach((row, rowIndex) => {
        if (row.length === 0 || (shiftIndex >= 0 && !row[shiftIndex])) {
            logger_1.Logger.debug('Skipping empty row:', rowIndex);
            return;
        }
        try {
            ...
            if (shiftStr.includes("–") || shiftStr.includes("-")) {
                ...
                if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
                    ...
                    logger_1.Logger.debug('Invalid range format in row:', rowIndex, shiftStr);
                    return;
                }
            }
            else {
                ...
                if (isNaN(shiftVal)) {
                    logger_1.Logger.debug('Invalid chemical shift in row:', rowIndex, shiftStr);
                    return;
                }
            ...
            const peak = new NMRPeak_1.NMRPeak(chemicalShift, multiplicity, jValues, integration);
            peaks.push(peak);
            logger_1.Logger.debug('Parsed peak from row:', rowIndex, peak);
        }
        catch (error) {
            logger_1.Logger.debug('Error parsing row:', rowIndex, error);
        }
    });
    const completeMetadata = new Metadata_1.Metadata(metadata.nuclei || "", metadata.solvent || "", metadata.frequency || 0);
    const result = new NMRData_1.NMRData(peaks, completeMetadata);
    logger_1.Logger.debug('Generated NMR data from table:', result);
    return result;
}
