...
export function multipletnumbers(multiplicityText: string): number[] | null {
    ...
    const multiplicityMap: { [key: string]: number } = {
        'd': 2,
        't': 3,
        'q': 4,
        'quint': 5
...
export function validateNMRPeak(peak: NMRPeak, index: number): ValidationError[] {
...
export function validateNMRData(nmrData: NMRData): ValidationError[] {
    const errors: ValidationError[] = [];
...
export function validateRichTextContent(textContent: string, parseFunction: (text: string) => NMRData): ValidationError[] {
    try {
